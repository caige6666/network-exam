...
function renderBank() {
  ...
  const items = slice.map(q => {
    ...
    return '<div class="q-item' + (exp ? " exp" : "") + '" data-id="' + q.id + '">' +
      ...
      "</div>" +
      '<div class="qi-stem" data-exp="' + q.id + '">' + esc(q.stem) + "</div>" +
  ...
  const bind = () => {
    ...
    $$("#view-bank [data-exp]").forEach(b => b.onclick = e => {
      ...
      if (item) { item.classList.toggle("exp"); const body = item.querySelector(".qi-body"); if (body) body.style.display = BankUI.exp === id ? "block" : "none"; const btn = item.querySelector('[data-exp="' + id + '"]'); if (btn) btn.textContent = BankUI.exp === id ? "收起解析" : "查看解析"; }
    });
